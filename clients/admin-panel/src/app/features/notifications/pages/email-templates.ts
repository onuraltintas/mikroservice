...
@Component({
  ...
  template: `
    <section class="space-y-6"><header class="flex flex-wrap items-center justify-between gap-3"><div><h1 class="text-2xl font-bold text-gray-900 dark:text-white">E-posta Şablonları</h1><p class="text-sm text-gray-500">Gönderim kodunu değiştirmeden konu ve HTML gövdesini kontrollü şekilde yönetin.</p></div><div class="flex gap-2"><button type="button" class="rounded-lg border px-4 py-2" [disabled]="loading()" (click)="load()">{{ loading() ? 'Yükleniyor…' : 'Yenile' }}</button><button type="button" class="rounded-lg bg-indigo-600 px-4 py-2 text-white" (click)="creating.set(!creating())">{{ creating() ? 'Vazgeç' : 'Yeni şablon' }}</button></div></header>
...
export class EmailTemplatesComponent {
  ...
  templates = signal<EmailTemplateDto[]>([]); selected = signal<EmailTemplateDto | null>(null); error = signal<string | null>(null); saving = signal(false); loading = signal(false);
  ...
  load() {
    ...
    this.service.getAll().pipe(finalize(() => this.loading.set(false))).subscribe({
      ...
      error: () => { this.templates.set([]); this.error.set('Şablonlar yüklenemedi. Lütfen tekrar deneyin.'); }
    });
  }
