...
@Component({
  ...
  template: `
    <section class="space-y-6">
      ...
      </tbody></table></div><div class="flex items-center justify-between text-sm text-gray-500"><span>Toplam {{ totalCount() }} talep · Sayfa {{ currentPage() }} / {{ totalPages() }}</span><div class="flex gap-2"><button class="rounded border px-3 py-1 disabled:opacity-40" [disabled]="currentPage() <= 1" (click)="changePage(currentPage() - 1)">Önceki</button><button class="rounded border px-3 py-1 disabled:opacity-40" [disabled]="currentPage() >= totalPages()" (click)="changePage(currentPage() + 1)">Sonraki</button></div></div>
    </section>
  `
})
export class SupportInboxComponent {
  ...
  currentPage = signal(1); readonly pageSize = 25;
  ...
  load() { this.service.getAll(this.currentPage(), this.pageSize, this.processed, this.search).subscribe({ next: response => { this.requests.set(response.items); this.totalCount.set(response.totalCount); }, error: () => this.error.set('Destek talepleri yüklenemedi.') }); }
