...
@Component({
  ...
  template: `
    <section class="space-y-6">
      ...
      <div class="flex flex-wrap gap-3"><input class="rounded-lg border p-2 dark:bg-gray-900" [(ngModel)]="search" (keyup.enter)="applyFilters()" placeholder="E-posta veya konu ara"><select class="rounded-lg border p-2 dark:bg-gray-900" [(ngModel)]="processed" (change)="applyFilters()"><option [ngValue]="undefined">Tümü</option><option [ngValue]="false">Açık</option><option [ngValue]="true">İşlenmiş</option></select><button class="rounded-lg border px-4 py-2" [disabled]="loading()" (click)="applyFilters()">{{ loading() ? 'Yükleniyor…' : 'Yenile' }}</button></div>
      ...
      </tbody></table></div><div class="flex items-center justify-between text-sm text-gray-500"><span>Toplam {{ totalCount() }} talep · Sayfa {{ currentPage() }} / {{ totalPages() }}</span><div class="flex gap-2"><button class="rounded border px-3 py-1 disabled:opacity-40" [disabled]="currentPage() <= 1 || loading()" (click)="changePage(currentPage() - 1)">Önceki</button><button class="rounded border px-3 py-1 disabled:opacity-40" [disabled]="currentPage() >= totalPages() || loading()" (click)="changePage(currentPage() + 1)">Sonraki</button></div></div>
    </section>
  `
})
export class SupportInboxComponent {
  ...
  currentPage = signal(1); readonly pageSize = 25; loading = signal(false);
  totalPages = computed(() => Math.max(1, Math.ceil(this.totalCount() / this.pageSize)));
  search = ''; processed: boolean | undefined; notes: Record<string, string> = {}; replies: Record<string, string> = {};
  constructor() {
  ...
  load() {
    ...
    this.error.set(null);
    this.service.getAll(this.currentPage(), this.pageSize, this.processed, this.search)
      .pipe(finalize(() => this.loading.set(false)))
      .subscribe({
        next: response => { this.requests.set(response.items); this.totalCount.set(response.totalCount); },
        error: () => { this.requests.set([]); this.totalCount.set(0); this.error.set('Destek talepleri yüklenemedi. Lütfen tekrar deneyin.'); }
      });
  }
  applyFilters() { this.currentPage.set(1); this.load(); }
  changePage(page: number) { if (page < 1 || page > this.totalPages()) return; this.currentPage.set(page); this.load(); }
