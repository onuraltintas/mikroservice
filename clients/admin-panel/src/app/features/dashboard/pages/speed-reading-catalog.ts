...
export class SpeedReadingCatalogComponent implements OnInit, OnDestroy {
  ...
  typeDraft: SpeedReadingExerciseTypeRequest = this.emptyType(); exerciseDraft: SpeedReadingExerciseRequest = this.emptyExercise(); readingTextDraft: SpeedReadingReadingTextRequest = this.emptyReadingText(); readingTextContent = ''; readingTags = ''; questionDraft: SpeedReadingReadingQuestionRequest = this.emptyQuestion(''); programDraft: SpeedReadingProgramTemplateRequest = this.emptyProgram(); adaptivePolicyDraft: AdaptivePolicyDraft = this.emptyAdaptivePolicy(); learningPathDraft: SpeedReadingLearningPathTemplateRequest = this.emptyLearningPath(); nodeDraft: SpeedReadingLearningPathNodeRequest = this.emptyNode(''); nodeContentDraft: SpeedReadingLearningPathNodeContentRequest = this.emptyNodeContent(''); achievementDraft: SpeedReadingAchievementRequest = this.emptyAchievement();
  ...
  startContentCreate(): void { if (this.contentTab() === 'exercise-types') { this.typeEditingId = null; this.typeDraft = this.emptyType(); this.typeEditing.set(true); } else if (this.contentTab() === 'exercises') { this.exerciseEditingId = null; this.exerciseDraft = this.emptyExercise(); this.exerciseEditing.set(true); } else { this.readingTextEditingId = null; this.readingTextDraft = this.emptyReadingText(); this.readingTextContent = ''; this.readingTags = ''; this.selectedReadingText.set(null); this.draftQualityMetrics.set(null); this.questionEditing.set(false); this.qualityPreviewError.set(''); this.readingTextEditing.set(true); } }
  ...
  private withAdaptivePolicy(value: string): string { try { const parsed = JSON.parse(value) as Record<string, unknown>; return JSON.stringify({ ...parsed, adaptation: this.adaptivePolicyDraft }); } catch { return JSON.stringify({ adaptation: this.adaptivePolicyDraft }); } }
