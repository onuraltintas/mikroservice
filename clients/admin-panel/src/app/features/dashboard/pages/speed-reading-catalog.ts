                ...
                <div class="dialog-panel dialog-panel-reading" role="dialog" aria-modal="true" aria-labelledby="reading-dialog-title" cdkTrapFocus [cdkTrapFocusAutoCapture]="true" (click)="$event.stopPropagation()">
                  ...
                  <div class="reading-workspace">
                    ...
                    <aside class="question-pane">
                      ...
                      @if (selectedReadingText()?.qualityMetrics; as quality) {
                        <section class="editor-section" aria-label="İçerik kalite ön kontrolü"><div class="section-heading"><div><h4>İçerik kalite ön kontrolü</h4><p class="muted">Türkçe metinler için Ateşman tahmini ve soru dağılımı. Uzman incelemesinin yerine geçmez.</p></div></div><div class="text-metrics"><span><strong>{{ quality.sentenceCount }}</strong> cümle</span><span><strong>{{ quality.averageWordsPerSentence }}</strong> kelime/cümle</span><span><strong>{{ quality.averageCharactersPerWord }}</strong> harf/kelime</span>@if (quality.estimatedAtesmanReadability !== null) {<span><strong>{{ quality.estimatedAtesmanReadability }}</strong> okunabilirlik · {{ quality.readabilityBand }}</span>}</div><p class="muted">Bloom: @for (item of quality.bloomDistribution; track item.level) { {{ item.level }}. seviye: {{ item.count }} } @empty { soru yok }</p><p class="muted">Zorluk: @for (item of quality.difficultyDistribution; track item.level) { {{ item.level }}: {{ item.count }} } @empty { soru yok }</p>@if (quality.warnings.length) {<ul class="mt-2 list-disc space-y-1 pl-5 text-sm text-amber-700 dark:text-amber-300">@for (warning of quality.warnings; track warning) {<li>{{ warning }}</li>}</ul>}</section>
                      }
