...
@Component({
  ...
  template: `
    <section class="space-y-6">
      ...
      @if (resource() === 'sessions') {
        <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"><table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700"><thead class="bg-gray-50 text-left text-xs uppercase text-gray-500 dark:bg-gray-900/40"><tr><th class="px-4 py-3">Başlık</th><th class="px-4 py-3">Tür</th><th class="px-4 py-3">Tarih</th><th class="px-4 py-3">Katılım</th><th class="px-4 py-3">Durum</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700">@for (item of sessions(); track item.id) {<tr><td class="px-4 py-3 font-medium">{{ item.title }}</td><td class="px-4 py-3">{{ item.sessionType }}</td><td class="px-4 py-3">{{ item.scheduledDate | date:'dd.MM.yyyy HH:mm' }}</td><td class="px-4 py-3">{{ item.presentCount }}/{{ item.studentCount }}</td><td class="px-4 py-3">{{ item.status }}</td></tr>} @empty {<tr><td colspan="5" class="px-4 py-10 text-center text-gray-500">Kayıt bulunamadı.</td></tr>}</tbody></table></div>
      } @else if (resource() === 'exams') {
        <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"><table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700"><thead class="bg-gray-50 text-left text-xs uppercase text-gray-500 dark:bg-gray-900/40"><tr><th class="px-4 py-3">Başlık</th><th class="px-4 py-3">Tür</th><th class="px-4 py-3">Tarih</th><th class="px-4 py-3">Maksimum puan</th><th class="px-4 py-3">Sonuç</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700">@for (item of exams(); track item.id) {<tr><td class="px-4 py-3 font-medium">{{ item.title }}</td><td class="px-4 py-3">{{ item.examType }}</td><td class="px-4 py-3">{{ item.examDate | date:'dd.MM.yyyy HH:mm' }}</td><td class="px-4 py-3">{{ item.maxScore }}</td><td class="px-4 py-3">{{ item.resultCount }}</td></tr>} @empty {<tr><td colspan="5" class="px-4 py-10 text-center text-gray-500">Kayıt bulunamadı.</td></tr>}</tbody></table></div>
      } @else {
        <div class="overflow-x-auto rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-800"><table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-gray-700"><thead class="bg-gray-50 text-left text-xs uppercase text-gray-500 dark:bg-gray-900/40"><tr><th class="px-4 py-3">Başlık</th><th class="px-4 py-3">Kategori</th><th class="px-4 py-3">İlerleme</th><th class="px-4 py-3">Hedef tarih</th><th class="px-4 py-3">Durum</th></tr></thead><tbody class="divide-y divide-gray-200 dark:divide-gray-700">@for (item of goals(); track item.id) {<tr><td class="px-4 py-3 font-medium">{{ item.title }}</td><td class="px-4 py-3">{{ item.category }}</td><td class="px-4 py-3">{{ item.currentProgress }}%</td><td class="px-4 py-3">{{ item.targetDate ? (item.targetDate | date:'dd.MM.yyyy') : '—' }}</td><td class="px-4 py-3">{{ item.isCompleted ? 'Tamamlandı' : 'Devam ediyor' }}</td></tr>} @empty {<tr><td colspan="5" class="px-4 py-10 text-center text-gray-500">Kayıt bulunamadı.</td></tr>}</tbody></table></div>
      }
