...
@Component({
  ...
  styles: [`
    ...
    .dialog-header h3 { font-size: 1.15rem; font-weight: 700; } .dialog-eyebrow { color: var(--ui-brand); font-size: .7rem; font-weight: 800; letter-spacing: .09em; text-transform: uppercase; }
    ...
    @media (max-width: 640px) { .dialog-backdrop { padding: .5rem; } .dialog-panel { max-height: calc(100vh - 1rem); } .form-grid { grid-template-columns: 1fr; } .wide { grid-column: auto; } }
  `]
})
export class SpeedReadingLanguageContentComponent implements OnInit {
  ...
  closeDialogOnEscape(): void {
    if (this.questionEditing()) this.cancelQuestionEdit();
    else if (this.vocabularyEditing()) this.cancelVocabularyEdit();
  }
