...
| Coaching | Global, bounded ve PII'siz operasyon özeti | `/api/coaching-admin/overview` | `SystemAdmin` global; `InstitutionAdmin/InstitutionOwner` kurum kapsamı + `Permissions.Coaching.View` |
| Coaching | Sayfalı assignment listesi; kaynak, kitap aralığı, teslim ve attachment sayaçları | `/api/coaching-admin/assignments` | `SystemAdmin` global; `InstitutionAdmin/InstitutionOwner` kurum kapsamı + `Permissions.Coaching.View` |
| Coaching | Assignment ayrıntısı; öğrenci teslim durumu, not/geri bildirim ve güvenli attachment metadata'sı | `/api/coaching-admin/assignments/{id}` | `SystemAdmin` global; `InstitutionAdmin/InstitutionOwner` kurum kapsamı + `Permissions.Coaching.View` |
...
| Coaching | Sayfalı seans, sınav ve hedef operasyon listeleri; arama/durum filtreleri | `/api/coaching-admin/sessions`, `/api/coaching-admin/exams`, `/api/coaching-admin/goals` | `SystemAdmin` global; `InstitutionAdmin/InstitutionOwner` kurum kapsamı + `Permissions.Coaching.View` |
| Coaching | Seans ve sınav ayrıntısı; katılım ve sonuç ölçüm alanları | `/api/coaching-admin/sessions/{id}`, `/api/coaching-admin/exams/{id}` | `SystemAdmin` global veya kurum kapsamı + `Permissions.Coaching.View`; kimlikler yalnız admin operasyonu için |
