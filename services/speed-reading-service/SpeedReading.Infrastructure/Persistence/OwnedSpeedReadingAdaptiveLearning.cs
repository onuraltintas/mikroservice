    ...
    : ISpeedReadingAdaptiveLearning
{
    ...
    public async Task<AdaptiveDashboardSummary> GetDashboardAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        ...
        var goal = await GetOrCreateDailyGoalAsync(userId, snapshot.UserProfile, cancellationToken);
        var todayGoal = BuildGoalSummary(goal, snapshot);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return levels
            ...
            .Select(level =>
            {
                ...
                return new AdaptiveWeakAreaSummary(level, BloomLevelName(level), score, recommendation);
            })
            .ToList();
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var recommendations = await db.AdaptiveContentRecommendations
            .AsNoTracking()
        ...
        var readingTextIds = recommendations.Select(item => item.ReadingTextId).Distinct().ToArray();
        ...
        var startedTextIds = await db.ReadingSessions
            .AsNoTracking()
            .Where(item => item.UserId == userId && readingTextIds.Contains(item.ReadingTextId))
        ...
        return recommendations
            ...
            .Select(item =>
            {
                ...
                return new AdaptiveContentRecommendationSummary(
                    ...
                    false,
                    isCompleted || startedTextIds.Contains(item.ReadingTextId),
                    isCompleted,
                    item.CreatedAt,
    ...
    public async Task<AdaptiveDailyGoalSummary> GetDailyGoalAsync(Guid userId, CancellationToken cancellationToken = default)
    {
        var snapshot = await LoadSnapshotAsync(userId, cancellationToken);
        var goal = await GetOrCreateDailyGoalAsync(userId, snapshot.UserProfile, cancellationToken);
        return BuildGoalSummary(goal, snapshot);
    }
    ...
    public async Task UpdateAfterSessionAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var profile = await db.AdaptiveLearningProfiles
        ...
        if (profile is null)
        {
            ...
            db.AdaptiveLearningProfiles.Add(profile);
        }
    ...
    private async Task<AdaptiveSnapshot> LoadSnapshotAsync(Guid userId, CancellationToken cancellationToken)
    {
        var profile = await db.AdaptiveLearningProfiles
            .AsNoTracking()
            .FirstOrDefaultAsync(item => item.StudentId == userId && !item.IsDeleted, cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var goal = await db.AdaptiveDailyGoals
        ...
        goal = new LegacyDailyGoal
        {
            ...
            StudentId = userId,
            Date = today,
            TargetMinutes = userProfile?.DailyGoalMinutes > 0 ? userProfile.DailyGoalMinutes : 20,
            CreatedBy = userId,
            CreatedAt = DateTime.UtcNow
        };
        db.AdaptiveDailyGoals.Add(goal);
        await db.SaveChangesAsync(cancellationToken);
        return goal;
    }

    private static AdaptiveDailyGoalSummary BuildGoalSummary(LegacyDailyGoal goal, AdaptiveSnapshot snapshot)
    {
        var today = DateTime.UtcNow.Date;
        var todayActivities = snapshot.Activities
            .Where(item => item.CompletedAt >= today && item.CompletedAt < today.AddDays(1))
            .ToList();
    ...
    private static AdaptiveProfileSummary BuildProfile(AdaptiveSnapshot snapshot)
    {
        ...
        return new AdaptiveProfileSummary(
            ...
            ParseLevels(snapshot.Profile?.WeakAreas),
    ...
    private static int CalculateCurrentStreak(IReadOnlyList<DateTime> activeDates)
    {
        if (activeDates.Count == 0 || activeDates[0] != DateTime.UtcNow.Date)
            return 0;
        var streak = 1;
        for (var index = 1; index < activeDates.Count && activeDates[index] == activeDates[index - 1].AddDays(-1); index++)
            streak++;
        return streak;
    }
