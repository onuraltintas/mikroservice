...
/// <summary>
/// Catalog queries backed by the owned Speed Reading store.
/// </summary>
internal sealed class OwnedSpeedReadingCatalog(OwnedSpeedReadingDbContext db)
    : ILegacySpeedReadingCatalog
{
        ...
        bool includeConfiguration = false)
    {
        ...
        if (!includeInactive)
            query = query.Where(item => item.exercise.IsActive && item.type.IsActive);

        if (exerciseTypeId.HasValue)
            query = query.Where(item => item.exercise.ExerciseTypeId == exerciseTypeId.Value);
        if (difficultyLevel.HasValue)
            query = query.Where(item => item.exercise.DifficultyLevel == difficultyLevel.Value);
        ...
        var items = await query
            ...
            .Select(item => new
            {
                ...
                TargetAgeGroupId = item.exercise.TargetAgeGroupId,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var item = await (
            ...
            where exercise.Id == id
                && !exercise.IsDeleted
                && !type.IsDeleted
                && (includeInactive || (exercise.IsActive && type.IsActive))
            select new
            {
                exercise.Id,
                exercise.Title,
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (exerciseId.HasValue)
            query = query.Where(item => item.ExerciseId == exerciseId.Value);
        if (!string.IsNullOrWhiteSpace(category))
            query = query.Where(item => item.Category == category);
        if (difficultyLevel.HasValue)
            query = query.Where(item => item.DifficultyLevel == difficultyLevel.Value);
        if (targetAgeGroupId.HasValue)
            query = query.Where(item => item.TargetAgeGroupId == targetAgeGroupId.Value);
        if (!string.IsNullOrWhiteSpace(searchTerm))
        {
            var pattern = $"%{searchTerm.Trim()}%";
            query = query.Where(item =>
