...
internal sealed class LegacySpeedReadingContentFeedback(SpeedReadingDbContext db)
    : ISpeedReadingContentFeedback
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        db.UserContentFeedbacks.Add(feedback);
    ...
    public async Task<ContentFeedbackAnalyticsSummary> GetAnalyticsAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var query = db.UserContentFeedbacks
            .AsNoTracking()
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        var currentLevel = Math.Clamp(user?.CurrentLevel ?? 1, 1, 10);
        ...
        if (normalizedType == "ReadingText")
        {
            ...
            return texts.Select(text => new RecommendedContentSummary(
                ...
                $"Seviye {text.DifficultyLevel} içeriği mevcut seviyenize ({currentLevel}) uygun."))
                .ToList();
        }
        ...
        if (normalizedType == "Exercise")
        {
            return await db.Exercises
                .AsNoTracking()
                .Where(item => !item.IsDeleted && item.DifficultyLevel >= currentLevel - 1 && item.DifficultyLevel <= currentLevel + 1)
        ...
        return await db.ExerciseProgramTemplates
            .AsNoTracking()
            .Where(item => !item.IsDeleted && item.IsActive)
    ...
    {
        var normalizedType = NormalizeContentType(contentType);
        return await db.UserContentFeedbacks
            .AsNoTracking()
    ...
    {
        var normalizedType = NormalizeContentType(contentType);
        var feedback = await db.UserContentFeedbacks
            .Where(item => item.UserId == userId
