...
public class ReplyToSupportRequestHandler : IRequestHandler<ReplyToSupportRequestCommand, Result>
{
    ...
    public async Task<Result> Handle(ReplyToSupportRequestCommand request, CancellationToken cancellationToken)
    {
        ...
        // 1. Send Email
        var subject = $"RE: {RemoveSubjectLineBreaks(supportRequest.Subject)} - Destek Talebi Cevabı";
        var firstName = WebUtility.HtmlEncode(supportRequest.FirstName);
        var lastName = WebUtility.HtmlEncode(supportRequest.LastName);
        var originalMessage = WebUtility.HtmlEncode(supportRequest.Message);
        var replyMessage = WebUtility.HtmlEncode(request.ReplyMessage);
        var body = $@"
            <html>
    ...
    private static string RemoveSubjectLineBreaks(string value) =>
